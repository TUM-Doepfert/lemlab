...
MACHINE = MACHINES[0]  # Machine that is currently worked on
...
def main(lemlab_scenario: str = './scenarios', lemlab_results: str = './simulation_results'):
    ...
    # Get a list of all topologies to simulate
    topologies = next(os.walk(lemlab_scenario))[1]
    for topology in topologies:
        top_path = os.path.join(lemlab_scenario, topology)
        markets = next(os.walk(top_path))[1]
        for market in markets:
            market_path = os.path.join(top_path, market)
            scens = next(os.walk(market_path))[1]
            scens = [os.path.join(market_path, scen) for scen in scens]
            scenarios.append(scens)

    # Flatten the list
    scenarios = [item for sublist in scenarios for item in sublist]

    # Create results list as well
    results = [os.path.join(lemlab_results, item.rsplit(os.sep)[-1]) for item in scenarios]
    ...
    for (scenario, result) in zip(scenarios, results):
        ...
        string = f"Running {scenario.rsplit(os.sep)[-1]}"
        pbar.set_description(string)

        try:
            sys.stdout = open(os.devnull, 'w')  # deactivate printing
            # Create a scenario executor
            simulation = ScenarioExecutor(path_scenario=scenario,
                                          path_results=result)
            # Run the scenario
            simulation.run()
